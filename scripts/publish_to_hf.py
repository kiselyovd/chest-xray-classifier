...
def main() -> None:
    ...
    parser.add_argument("--repo-id", default="kiselyovd/chest-xray-classifier")
    ...
    parser.add_argument(
        "--base-model",
        default="",
        help="HF base model ID (e.g. facebook/convnextv2-tiny-22k-224).",
    )
    parser.add_argument(
        "--hf-dataset",
        default="",
    ...
    parser.add_argument(
        "--domain-tag",
        default="",
        help="Domain tag (e.g. medical-imaging).",
    )
    parser.add_argument(
        "--pipeline-tag",
        default="",
        help="HF pipeline tag (e.g. image-classification).",
    )
    parser.add_argument(
        "--library-name",
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for item in artifacts_dir.rglob("*"):
            ...
            # Skip raw Lightning checkpoints (unusable by from_pretrained) and the
            # hf_export/ subdir (its contents are copied to the repo root below, so
            # uploading it here would duplicate the weights under hf_export/).
            if item.suffix == ".ckpt" or rel.parts[0] == "hf_export":
                continue
            dest = tmp_path / rel
        ...
        render_model_card(
            ...
            github_url="https://github.com/kiselyovd/chest-xray-classifier",
