...
evaluate:
	uv run python -m chest_xray_classifier.evaluation.evaluate --checkpoint artifacts/checkpoints/best.ckpt --data data/processed

serve:
	MODEL_PATH=artifacts/checkpoints/best.ckpt uv run uvicorn chest_xray_classifier.serving.main:app --reload --port 8000
test:
	uv run pytest
